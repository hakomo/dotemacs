...
//snippet-begin snippet-name="inc" snippet-only
#include<deque>
#include<list>
#include<map>
#include<queue>
...
//snippet-begin snippet-name="random"
int random(int mx){
...
//snippet-begin snippet-name="combi"
...
//snippet-begin snippet-name="bit"
template<class T>
struct Bit{
  vector<T> a;
  Bit(int n):a(n){}
...
//snippet-begin snippet-name="sieve"
template<class T>
...
//snippet-begin snippet-name="isprime"
...
//snippet-begin snippet-name="factor"
struct Factor{
  vector<int> a;
...
//snippet-begin snippet-name="pow"
/*
struct Power{
  vector<int> a;
...
//snippet-begin snippet-name="radix"
struct Radix{
  const char* s;
  int a[128];
...
//snippet-begin snippet-name="split"
...
//snippet-begin snippet-name="print"
template<class T>
...
//snippet-begin snippet-name="miex" snippet-only
for(int _i=0;_i<6;++_i){
  MatrixIterator _m( , ,_i),_mi=_m.begin(),_mj=_m.end();
  for(;_mi!=_mj;++_mi){
    MatrixIterator _ai=_mi.begin(),_aj=_mi.end();
    for(;_ai!=_aj;++_ai){
      [_ai.y][_ai.x];
    }
  }
...
//snippet-begin snippet-name="bs" snippet-only
int T=,F=;
while(abs(T-F)>1)
  (f((T+F)/2)?T:F)=(T+F)/2;
//snippet-end
